...
while(state):
    ...
    for (x, y, w, h) in faces:  
        ...
        # Se triplican los canales de color
        for index, item in enumerate(vg_input):
            item[:, :, 0] = np_image_array
            item[:, :, 1] = np_image_array
            item[:, :, 2] = np_image_array  
        
        # Inferencia       
        result = CNN_model.predict(vg_input)        
